...
public class RegisterUserCommandHandler : ICommandHandler<RegisterUserCommand, SuccessMessageAndObjectId>
{
    ...
    {
        var result = await Validate(request, cancellationToken);
    ...
    private async Task<OperationResult<SuccessMessageAndObjectId>> Validate(RegisterUserCommand command, CancellationToken cancellationToken)
    {
        var result = new OperationResult<SuccessMessageAndObjectId>();

        var userExists = await _context.Users
            .AnyAsync(user => user.Login == command.Login
                || user.Email == command.Email, cancellationToken);

        if (userExists)
        {
            result.AddError(AuthenticationsRescource.UserWithEmailOrLoginAlreadyExists_ErrorMessage);
        }
